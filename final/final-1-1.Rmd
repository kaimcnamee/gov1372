...
joined_data %>% 
  ...
  summarize(asians = mean(asians),
            tot_cases = mean(tot_cases), .groups = "drop") %>% 
  ggplot(aes(x = tot_cases, y = asians)) +
    geom_point() +
...
```{r models}
m1_states <- lm(asians ~ fox + cnn + party + date + tot_cases + new_case + tot_death + new_death + state + lockdown + search_coronavirus + search_china_virus, joined_data %>% drop_na(tot_cases))

m1 <- lm(asians ~ fox + cnn + party + date + tot_cases + new_case + tot_death + new_death + lockdown + search_coronavirus + search_china_virus, joined_data %>% drop_na(tot_cases))
