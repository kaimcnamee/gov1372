...
heat_data_2 <- NationscapeVars %>% 
  ...
  select(ideo5, interest, right_track, economy_better, extra_priorities_crime, registration, news_sources_cnn,
         news_sources_fox)
```
...
Difference in means shows there is a significant difference in favorability towards asians before and after the shooting, but the effect size is negligible.  
...
full_data_clean %>% 
  ...
  summarize(group_favorability_asians = mean(group_favorability_asians),
            .groups = "drop") %>% 
  mutate(treatment = as.factor(ifelse(day_running < 0, "Before", "After"))) %>% 
  ggplot(aes(x = day_running, y = group_favorability_asians)) +
    ...
    theme_light() +
    labs(title = "Anti-Asian sentiment over time") +
    scale_color_discrete(name = "", labels = c("Before" = "Before pandemic",
...
full_data_clean %>% 
  ...
  ggplot(aes(x = day_running, y = group_favorability_asians)) +
    ...
    facet_wrap("pid3") +
    theme_light() +
    labs(title = "Anti-Asian sentiment by party") +
    scale_color_discrete(name = "", labels = c("Before" = "Before pandemic",
                                               "After" = "After pandemic")) +
    geom_vline(xintercept = 0, linetype = "dashed")
...
full_data_clean %>% 
  ...
  ggplot(aes(x = day_running, y = group_favorability_asians)) +
    ...
    facet_wrap("news") +
    theme_light() +    
    labs(title = "Anti-Asian sentiment by news source") +
    scale_color_discrete(name = "", labels = c("Before" = "Before pandemic",
                                               "After" = "After pandemic")) +
    geom_vline(xintercept = 0, linetype = "dashed")
...
full_data_clean %>% 
  ...
  ggplot(aes(x = day_running, y = group_favorability_asians)) +
    ...
    theme_light() +    
    labs(title = "Anti-Asian sentiment by racial group") +
    scale_color_discrete(name = "", labels = c("Before" = "Before pandemic",
                                               "After" = "After pandemic")) +
    geom_vline(xintercept = 0, linetype = "dashed")
...
preds_before <- tibble(group_favorability_asians = preds_before$fit,
                       day_running = pred_data$day_running,
                       treatment = F)
...
# Visualize
preds_combined %>% 
  ggplot(aes(x = day_running, y = group_favorability_asians, color = treatment)) +
    geom_path() +
