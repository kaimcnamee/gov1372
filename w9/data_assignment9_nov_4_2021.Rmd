...
data_4c <- data_h1 %>% 
  ...
  select(treat_rand1, requested_more, sum_correct, mean_disgust, mean_anxious, page_article_timing:mean_anxious) %>% 
  rename(disgust = mean_disgust, anxious = mean_anxious)

data_4c

m1 <- lm(requested_more ~ disgust + anxious + sum_correct + female, data = data_4c)

# Read time and correct answers are not statistically significant
m2 <- lm(sum_correct ~ page_article_timing, data = data_4c)

m3 <- lm(sum_correct ~ disgust + anxious, data = data_4c)
...
coef_plot <- as_tibble(confint(m1)) %>% 
  mutate(variable = c("constant", "disgust", "anxious", "sum_correct", "female"),
         coef = c(0.151, -0.021, 0.117, -0.018, 0.018)) %>% 
  rename("lower" = `2.5 %`,
         "upper" = `97.5 %`)
  
coef_plot

level_order <- rev(c("disgust", "anxious", "sum_correct", "female", "constant"))
