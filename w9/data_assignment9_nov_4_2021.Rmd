...
data_h1 <- data_clean %>% 
  select(treat_rand1, disgust, anxious, contains("symptoms")) %>% 
  ...
  mutate(check_fatigue = ifelse(symptoms_fatigue == ans_symptoms_fatigue, 1, 0),
         ...
         sum_check = rowSums(across(check_fatigue:check_fever))) %>% 
  select(treat_rand1, disgust, anxious, sum_check)

data_h1 %>% 
  drop_na() %>% 
  mutate(across(treat_rand1:sum_check, as.numeric)) %>% 
  pivot_longer(cols = disgust:anxious, names_to = "type", values_to = "value") %>% 
  ggplot(aes(x = value, y = sum_check, color = type)) +
    geom_jitter() +
    facet_wrap("type")
```
